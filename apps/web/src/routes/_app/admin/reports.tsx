  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Pedidos" value={summary?.totalOrders ?? 0} />
        <StatCard label="Ingresos" value={`$${Number(summary?.totalRevenue ?? 0).toFixed(2)}`} color="text-success" />
        <StatCard label="Impuestos cobrados" value={`$${Number(summary?.totalTax ?? 0).toFixed(2)}`} />
        <StatCard label="Descuentos otorgados" value={`$${Number(summary?.totalDiscounts ?? 0).toFixed(2)}`} color="text-destructive" />
      </div>
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        ...
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={revenueData}>
            ...
            />
            <Line type="monotone" dataKey="revenue" stroke="#f59e0b" strokeWidth={2} dot={false} name="Ingresos ($)" />
          </LineChart>
        </ResponsiveContainer>
      </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={topItems.map((i: any) => ({ name: i.itemName.substring(0, 12), qty: Number(i.totalQuantity) }))} layout="vertical">
              ...
              <Bar dataKey="qty" fill="#f59e0b" name="Cant. vendida" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="bg-surface border border-border rounded-xl p-4">
          <h2 className="font-semibold mb-4">Ventas por mesero</h2>
            ...
            {byWaiter.map((w: any) => (
              <div key={w.waiterId} className="flex items-center justify-between text-sm">
                ...
                <div className="text-right">
                  ...
                  <div className="text-muted text-xs">{w.orderCount} pedidos</div>
                </div>
              </div>
            ))}
            {byWaiter.length === 0 && <p className="text-muted text-sm">Sin datos para este período</p>}
          </div>
        </div>
      </div>
      ...
      {/* Inventory usage */}
      <div className="bg-surface border border-border rounded-xl p-4">
        <h2 className="font-semibold mb-4">Uso de ingredientes</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border text-left text-muted">
                ...
                <th className="pb-2">Desperdiciado</th>
      ...
      {lowStock.length > 0 && (
        <div className="bg-destructive/10 border border-destructive/30 rounded-xl p-4">
          <h2 className="font-semibold text-destructive mb-3">Stock bajo actual</h2>
